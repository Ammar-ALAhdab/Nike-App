      ...
      <div className="flex justify-center items-start gap-20 flex-wrap max-lg:flex-col">
        <div className="flex flex-col items-start">
          ...
          <p className="mt-6 text-base leading-7 font-montserrat text-white-400 sm:max-w-sm">
            ...
            pariatur atque delectus in est eius soluta asperiores.
          </p>
          <div className="flex items-center gap-5 mt-8">
            {socialMedia.map((icon) => (
              <div key={icon.alt} className="flex w-12 h-12 items-center justify-center bg-white rounded-full ">
            ...
            <div key={section.title}>
                ...
                <ul>
                  {section.links.map(link => (
                    <li key={link.name} className="mt-3 text-white-400 cursor-pointer font-montserrat text-base leading-normal hover:text-slate-gray"><a href={link.link}>{link.name}</a></li>
                  ))}
                </ul>
            </div>
          ))}
